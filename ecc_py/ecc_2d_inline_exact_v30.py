...
__global__ void ECC_kernel_v30(
    ...
    // Calculate pixel position (1-based indexing like original)
    ...
    if (ix < imageW + 1 && iy < imageH + 1) {
        ...
        float t = (iy > 1) ? data[idx - imageW] : 1e30f;           // top
        float b = (iy < imageH) ? data[idx + imageW] : -1e30f;     // bottom
        float l = (ix > 1) ? data[idx - 1] : 1e30f;                // left
        float r = (ix < imageW) ? data[idx + 1] : -1e30f;          // right
        float tl = (ix > 1 && iy > 1) ? data[idx - imageW - 1] : 1e30f;     // top-left
        float tr = (ix < imageW && iy > 1) ? data[idx - imageW + 1] : 1e30f; // top-right
        float bl = (ix > 1 && iy < imageH) ? data[idx + imageW - 1] : -1e30f; // bottom-left
        float br = (ix < imageW && iy < imageH) ? data[idx + imageW + 1] : -1e30f; // bottom-right

        // Algorithm logic from v30 - EXACT COPY
        bool c_l_l = (c < l);
        bool c_lq_r = (c <= r);
